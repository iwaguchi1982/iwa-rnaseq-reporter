...
def build_report_identity(plan: ResolvedComparisonPlan) -> ReportIdentitySpec:
    ...
    return ReportIdentitySpec(
        ...
        comparison_column=str(plan.metadata.get("comparison_column", "unknown")),
...
def build_report_summary_snapshot(
    ...
    if padj_threshold is not None and abs_log2_fc_threshold is not None:
        ...
        for r in result_spec.rows:
            # Check p-value/q-value and effect size
            # We use q_value (padj) for significance
...
def build_report_display_context_snapshot(
    ...
    return ReportDisplayContextSnapshot(
        ...
        matrix_kind=str(plan.metadata.get("matrix_kind", "unknown")),
        normalization=plan.normalization
    )

def build_default_narrative_slots() -> List[ReportNarrativeSlot]:
    """
...
def build_report_payload_spec(
    ...
    abs_log2_fc_threshold: float = 1.0,
    sort_by: str = "padj",
    preview_top_n: int = 100
) -> ReportPayloadSpec:
    """
    Main entry point for assembling the ReportPayloadSpec (SOT).
    """
    ...
    narrative_slots = build_default_narrative_slots()
    ...
    return ReportPayloadSpec(
        ...
        project_id="UNKNOWN", # Default project ID
        ...
        artifacts=[
            ...
            # Placeholder for plots when generated by pipeline later
        ],
        metadata={
            "app_version": "0.4.0",
